...
int process_text(const char *pathToFile, int bufferSize) {
    int fd = open(pathToFile, O_RDONLY);
    if (fd < 0) {
        perror("Error opening file");
        exit(EXIT_FAILURE);
    ...
    while ((bytesRead = read(fd, read_buffer, bufferSize)) > 0) {
        for (ssize_t i = 0; i < bytesRead; i++) {
            while (buffer_free_bytes(&buffer) == 0) { // buffer full
                ...
                if (elemSize == -1) {
                    fprintf(stderr, "Num bigger than the buffer, no available\n");
                ...
                if (elemSize >= (int)sizeof(temp_num)) {
                    fprintf(stderr, "Num bigger than the temporary space number, no available\n");
                    buffer_deallocate(&buffer);
                    close(fd);
                    exit(EXIT_FAILURE);
                }
                ...
                suma += atoll(temp_num); //better than atoi for big numbers
